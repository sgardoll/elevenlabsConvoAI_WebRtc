...
class _SimpleRecordingButtonState extends State<SimpleRecordingButton>
  ...
  void _setupServiceListeners() {
    ...
    _recordingSubscription = _service.recordingStream.listen((isRecording) {
      if (mounted) {
        print('🔄 Button received recording state update: $isRecording');
        setState(() {
    ...
    _stateSubscription = _service.stateStream.listen((state) {
      if (mounted) {
        print('🔄 Button received conversation state update: $state');
        setState(() {
          _currentState = state;
        });
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted) {
            final shouldBeAnimating =
                state == ConversationState.recording && widget.pulseAnimation;
  ...
  Future<void> _handleTap() async {
    ...
    try {
      print(
          '👆 Button tapped, current state: $_currentState, recording: $_isRecording');
